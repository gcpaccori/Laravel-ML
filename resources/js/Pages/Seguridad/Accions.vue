<script setup>
    ...
    const ajaxUrl = route('datatable.accions');
    ...
    const handleEdit = async (id) => {
        const { data } = await axios.get(route('seguridad.accions.edit', id));
    ...
    const handleDelete = async(id) => {
        ElMessageBox.confirm(
            ...
            try {
                const response = await axios.delete(route('seguridad.accions.destroy', id));
                ElMessage({
...
<template>
    <App :title="title" :toolbar="toolbar">
            ...
            <template #filters>
                <el-form :model="formInline" label-position="top" class="w-100">
                    <el-row :gutter="20">
                        ...
                        <el-col :lg="6">
                            <el-form-item label="Nombre Función">
                                <el-input
                                    v-model="formInline.f_name_funcion"
                                    type="text"
        ...
        <FormAccions v-model="dialogVisible" :dataForm="dataForm" @saved="handleSaved"/>
    </App>
</template>
