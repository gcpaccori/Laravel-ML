<script setup>
    ...
    import EspecieForm from "../Form/EspecieForm.vue";
    ...
    const ajaxUrl = route('datatable.especies');
    ...
    const handleEdit = async (id) => {
        const { data } = await axios.get(route('especies.edit', id));
        dataForm.value = data;
        dialogVisible.value = true;
    };
    ...
    const handleDelete = async(id) => {
        ElMessageBox.confirm(
            ...
            try {
                const response = await axios.delete(route('especies.destroy', id));
                ElMessage({
...
<template>
    <App :title="title" :toolbar="toolbar">
        ...
        <EspecieForm v-model="dialogVisible" :dataForm="dataForm" @saved="handleSaved"/>
    </App>
</template>
